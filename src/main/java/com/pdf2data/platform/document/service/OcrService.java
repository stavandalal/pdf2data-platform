...
public class OcrService {
    ...
    public OcrService() {
        ...
        try {
            ...
            throw new RuntimeException(
                    "Unable to initialize Tesseract",
                    e
            );
            ...
            for (int x = 1; x < image.getWidth() - 1; x++) {
                ...
                for (int j = -1; j <= 1; j++) {
                    ...
                    for (int i = -1; i <= 1; i++) {
                        ...
                        if ((image.getRGB(x + i, y + j) & 0xFFFFFF) == 0) {
                            blackNeighbours++;
                        }
                ...
                if (blackNeighbours >= 5) {
                    cleaned.setRGB(x, y, Color.BLACK.getRGB());
                } else {
    ...
    private void validateImage(BufferedImage image) {
        ...
        if (image.getWidth() < 30 || image.getHeight() < 30) {

            throw new RuntimeException(
                    "Image resolution is too small for OCR."
            );
